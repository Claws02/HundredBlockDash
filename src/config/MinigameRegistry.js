...
export const MG_INFO = {
    ...
    rooftop:     { icon: '🌃', title: 'ROOFTOP RUN', desc: 'Turn the phone sideways and sit side by side. You are both racing across the Back Alley rooftops at night — you run on your own. SWIPE UP on your half to JUMP the gaps and the vents (keep your finger down for a higher jump); SWIPE DOWN to SLIDE under the low neon signs. Vents carry a yellow JUMP board and signs a blue SLIDE board. Slide into the runner in front and you trip them up! Miss a gap and you drop to the street and lose time. First to the finish banner wins the race; first to 2 races wins!' },
    blockparty:  { icon: '🪩', title: 'BLOCK PARTY', desc: 'Turn the phone sideways and sit side by side. It is a street party on the Promenade and the DJ calls the moves: watch the DJ dance a phrase, one move per beat — then dance it straight back, both of you at once. SWIPE UP to raise the roof, DOWN to drop, LEFT or RIGHT to point, TAP to clap. Dance to the music: on the kick drum is PERFECT, near it is GOOD, a bit early or late is still OK; only the wrong move (or none at all) is a MISS. A phrase with no misses is a FULL COMBO. The phrases get longer and faster, and the last two are from memory. Most points wins!' },
    ...
    balloonpump: { icon: '🎈', title: 'BALLOON PUMP', desc: 'Turn the phone sideways and sit side by side. HOLD your thumb on your half to pump your balloon, and let go to stop. When the whistle blows, the bigger balloon scores. Pump past the limit and it POPS for nothing, and the round ends right there: the other balloon wins it. Both balloons share the same hidden limit, and a balloon near it starts to wobble and squeak. Five rounds, and the last one counts double!' },
    ...
    kartgp: { icon: '🏁', title: 'GO-KART GRAND PRIX', desc: 'Lay the phone flat between you. The screen splits, and each half is your own camera behind your own kart. Each race is on one of five circuits, picked at random: the Park Circuit, Canyon Switchbacks, Beach Boardwalk (with boost pads), Snowy Mountain (mind the black ice) and Neon City Night. The kart drives itself: DRAG left and right on your half to steer. Steer hard through a corner to DRIFT, and straighten up for a turbo. Drive through the ? boxes and TAP to use what you get: a 🍄 mushroom (a huge burst of speed) or a 🍌 banana to drop behind you. Some circuits have a shortcut, if you can hit it. First to finish 3 laps wins!' },
    ...
    tag: { icon: '🏃', title: 'TAG, YOU\'RE IT!', desc: 'Lay the phone flat between you. One of you is IT — the yellow ring — and a touch quicker: touch the other to pass it on. The clock runs on whoever is IT, and after 45 seconds whoever was IT for the LEAST time wins. Every game is on one of four maps: the Playground, the Building Site, the Backyard or the Snowy Park. Climb a ladder or ramp onto a deck to get out of reach (IT has to climb up after you), then take the slide down fast to escape. Watch out for sand, the paddling pool and the ice!' },
    ...
    minigolf: { icon: '⛳', title: 'MINI GOLF', desc: 'Turn the phone sideways and sit side by side: the whole hole is on screen and you both play it AT THE SAME TIME. DRAG BACK anywhere on YOUR HALF of the screen and LET GO to putt your ball — the further you pull, the harder you hit — and go again as soon as your ball stops. The balls knock into each other, so you can bump your rival off line. Three holes, picked at random from seven: the windmill, the loop, the fountain bridge, pinball bumpers, the dogleg, sand traps and the slider. Fewest strokes wins; level on strokes, the quicker time wins.' },
    bowling: { icon: '🎳', title: 'BOWLING', desc: 'Lay the phone flat between you. Two neon lanes run side by side, one each, and the screen splits so each half is your own view down your lane. DRAG SIDEWAYS to slide the ball across and line up, then FLICK up toward the pins to bowl: a faster flick rolls a faster ball, and bending your flick puts spin on it: bend off to the right and the ball hooks LEFT, bend left and it hooks right. No clock, take your time. Watch the gutters! Two frames each, played together, with proper strikes and spares, and a strike or spare in the last frame earns bonus balls. Most pins wins, and if you are level a tie-break frame decides it!' },
...
export const MG_WATCHDOG_MS = {
    ...
    minigolf:    240000,   // three holes, both at once: ~1–2 min is a normal round
...
export const MG_PROFILE = {
    ...
    // A tap is a discrete event and the carts run on rails at a fixed speed,
    // so the whole race replays from the taps: 'events'. Two carts is the
    // story for now; four on this grid would jam every junction.
    minecart:    { genre: 'race',     control: 'tap',   wire: 'events',   seats: [2, 2], live: false },
