...
let _dirOwns = false, _cards = null, _lastIn = null, _hold = false;
...
function _destroy() {
    ...
    _figs = []; _dj = null; _set = null; _dir = null; _hud = null; _in = null; _fx = null; _cards = null;
}
...
function _input(slot, move, at) {
    ...
    _judge(slot, i, j, move !== p.moves[i] ? 'WRONG MOVE' : '');
}
    ...
    if (_phase === 'dance' && _ph >= 0) {
        ...
        if (b !== _lastBeat) {
            ...
            if (w.part === 'resp' && w.i === 0) {
                _hud.say('YOUR TURN!', '', 700, _t, '#34f5a0');
...
function _buildCards() {
    ...
    row.style.cssText = 'position:absolute;left:50%;top:50px;transform:translateX(-50%);display:flex;gap:8px;pointer-events:none;';
    _stage.hud.appendChild(row);
    _cards = { row, list: [] };
}
function _layCards(p) {
    if (!_cards) return;
    _cards.row.innerHTML = '';
    _cards.list = p.moves.map(m => {
        const c = document.createElement('div');
        c.style.cssText = 'width:40px;height:50px;border-radius:9px;background:rgba(20,12,24,.7);border:2px solid rgba(255,255,255,.25);' +
            'display:flex;flex-direction:column;align-items:center;justify-content:center;transition:transform .08s,opacity .15s;opacity:.35;';
        const g = document.createElement('div');
        g.style.cssText = 'font-size:22px;line-height:26px;color:#fff;visibility:hidden;';
        g.textContent = GLYPH[m];
        const dots = document.createElement('div');
        dots.style.cssText = 'display:flex;gap:6px;margin-top:3px;';
        const d = [1, 0].map(slot => {
            const e = document.createElement('div');
            e.style.cssText = `width:9px;height:9px;border-radius:50%;border:2px solid ${seat(slot).css};`;
            dots.appendChild(e);
            return e;
        });
        c.append(g, dots);
        _cards.row.appendChild(c);
        return { c, g, dot: [d[1], d[0]] };     // dot[slot]
    });
}
function _revealCard(i) {
...
function _darkCards() { _cards?.list.forEach(k => { k.g.textContent = '?'; k.g.style.visibility = 'visible'; k.g.style.opacity = '.5'; }); }
...
function _paintDots() {
    _cards?.list.forEach((k, i) => [0, 1].forEach(slot => {
        const j = _resp[slot][i];
        k.dot[slot].style.background = j ? JUDGE_CSS[j] : 'transparent';
    }));
}
