...
| `./parsecheck.sh ../src` | Module-aware syntax gate. `node --check` parses a file as CommonJS and tolerates things that are invalid in an ES module — it silently passed a dangling `else` in GameController. This copies each file to `.mjs` first so Node parses it as what it is. **Run before anything else.** | ~2 s |
