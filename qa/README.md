...
| `turfwar.js` | Turf War: a real drag runs P1 and paints the yard, the rival's wet paint is slower than bare concrete (measured in game time), a paint bomb splats a patch, and a hard bot out-paints an idle player. Writes `shot-turfwar-*.png`. |
| `lilypad.js` | Lily Pad Leap: a real drag aims and letting go hops, landing on the rival stomps them off their pad, a pad sinks under a figure who stands still, and a hard bot beats an idle player. Writes `shot-lilypad-*.png`. |
| `rooftop.js` | Rooftop Run: a real tap on the top of P1's half jumps and holding goes higher, standing up into a sign stumbles and a real slide passes under it, a slide trips the runner ahead, running off a roof drops to the street and climbs back up, and a hard bot beats an idle player. Writes `shot-rooftop-*.png`. |
| `blockparty.js` | Block Party: the DJ dances the call, real swipes dance up/down/left/right, a real swipe of the called move on a held beat is a hit, exact-beat moves judge PERFECT / GOOD / MISS, no move is a miss, a clean phrase pays the combo, and a hard bot beats an idle player. Writes `shot-blockparty-*.png`. |
| `riftdive.js` | Rift Dive: the dive is split into two views; a real drag on each half steers that diver the way it points on its own turned view; a ring counts and boosts, a shard stuns, side-by-side divers bump apart, and a hard bot beats an idle player. Writes `shot-riftdive-*.png`. |
