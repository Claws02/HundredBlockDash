...
| `mgMusic(mode)` (`AudioManager.js`) | For a game whose rule is the music (Musical Chairs): `'play'` runs the band through the minigame gate, `'dip'` is a fake-out, `'stop'` cuts it dead that instant, `'off'` hands it back to the board. Call `'off'` from `_destroy`. |
